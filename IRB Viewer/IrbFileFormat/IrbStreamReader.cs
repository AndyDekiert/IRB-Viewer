namespace IRB_Viewer.IrbFileFormat
{
    class IrbStreamReader
    {
        ...
        public IrbStreamReader(string filename)
        {
            this.m_eof = true;
            this.m_offset = 0;
        ...
        private void Open(string filename)
        {
            ...
            {
                Console.Error.WriteLine($"IO.BinaryReader fail. {ex.Message}, {filename}");
                return;
            }
        ...
        public byte[] ReadByte(int length, int offset)
        {
            ...
            if (m_offset < m_blocklen)
            {
                ...
                {
                    Console.Error.WriteLine($"readByte(): length: {length} / offset: {offset} - {e.Message}");
                    length = 0;
                    m_eof = true;
                }
            ...
            else
            {
                if (length > 0)
                {
                    Console.Error.WriteLine("ReadStr:EOF!");
                    length = 0;
                }
