...
namespace IRB_Viewer.IrbFileFormat
{
    class IrbFileReader
    {
        ...
        private IrbStreamReader reader;
        ...
        public IrbFileReader(string filename)
        {
            ...
            reader = new IrbStreamReader(filename);


            Head.MagicNumber = reader.ReadByte(5, -1);

            //- ID
            if (!Head.MagicNumber.SequenceEqual(new byte[] { 255, 73, 82, 66, 0 })) //-- soll "\xFF" "IRB" "\0" aber C schneidet das \0 weg!
            {
                Console.Error.WriteLine("Irb File - ''Magische Number'' wrong");
                return;
            }
            ...
            {
                Console.Error.WriteLine("Unknown Irbis File Type");
                return;
            }
       ...
       /// </summary>
        public byte[] GetImageData(int imageIndex)
        {
            ...
            if (blockIndex < 0)
            {
                Console.Error.WriteLine($"getImageData(imageIndex) fail - ImageIndex: {imageIndex} not found");
                return null;
            }
