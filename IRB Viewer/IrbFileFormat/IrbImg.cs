namespace IRB_Viewer.IrbFileFormat
{
    class IrbImg
    {
        ...
        {
            if (Data == null) Console.Error.WriteLine("getData() Accessing non initialised data!");
            return Data;
        }
        ...
        public bool ReadImage(int imageIndex)
        {
            ...
            var reader = new IrbBufferReader(this.reader.GetImageData(imageIndex));
            ...
            {
                Console.Error.WriteLine("??? value != (Height - 1)");
            }
            ...
            {
                Console.Error.WriteLine("Defect Irbis Image File: Image Width(" + Width + ") or Height(" + Height + ") is out of range!");
                Width = 1;
                Height = 1;
                return false;
            }
            ...
            if (reader.Eof) Console.Error.WriteLine("end of file!");
        ...
        /// </summary>
        public void ReadFlags(IrbBufferReader reader, int offset)
        {
        ...
        /// </summary>
        private float[] ReadPallet(IrbBufferReader reader, int offset)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private float[] ReadImageData(IrbBufferReader reader, int bindata_offset, int width, int height, int palette_offset, int useCompression)
        {
            ...
            if (!useComp)
            {
                for (int i = pixelCount; i > 0; i--) {
                    ...
                    // v = Palette[Math.Min(Palette.Length - 1, v2 + 1)] * f + Palette[v2] * (1.0f - f);
